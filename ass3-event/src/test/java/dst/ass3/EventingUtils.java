...
public final class EventingUtils {
	...
	/**
	 * Checks if {@code lectureId} from bean matches ANY in {@code compareWith}.
	 ...
	 *            the event bean
	 * @param compareWith
	 *            the list of possible lectureIds
	 */
	public static void ensureLectureId(String failMsg, EventBean e,
			long... compareWith) {
		StringBuffer couldBe = new StringBuffer();
		try {

			Long id = getLong(e, Constants.EVENT_PROP_LECTURE_ID);
			...
			fail(String.format("%s : lectureId should be in ( %s) but was %d",
					failMsg, couldBe, id));
		} catch (Exception b) {
			b.printStackTrace();
		}
